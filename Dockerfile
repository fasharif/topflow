...
# Apply pending migrations (guarded by Prisma's advisory lock), then start the API.
CMD ["sh", "-c", "npm run db:deploy -w @topflow/database && exec node apps/api/dist/main.js"]
