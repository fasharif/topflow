...
function WelcomeBanner({ firstName }: { firstName: string }) {
  ...
  return (
    <Alert tone="success" title={`Welcome to Top Flow, ${firstName}!`}>
      Your account is ready. Browse the catalogue, check out with payment on delivery, and follow every order right here.
    </Alert>
  );
}
...
function TradePortalCard({ memberships }: { memberships: MembershipSummary[] }) {
  return (
    <Card tone="brand" className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex min-w-0 gap-4">
        <span className="grid size-10 shrink-0 place-items-center rounded-lg bg-white text-brand-700 ring-1 ring-inset ring-brand-200">
          <Building aria-hidden="true" className="size-5" />
        </span>
        <div className="min-w-0">
          <h2 className="heading-4 text-ink-900">Trade account</h2>
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {memberships.map((membership) => (
              <li
                ...
                className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 text-xs text-slate-600 ring-1 ring-inset ring-brand-200"
              >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
        ...
        <SecurityCard email={user.email} mfaRequired={user.mfaRequired} />
      </div>
    </div>
  );
}
